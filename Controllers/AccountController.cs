    ...
    public class AccountController : Controller
    {
        ...
        public IActionResult Login(LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                User user = _dbContext.Users.FirstOrDefault(x => x.Username.ToLower() == model.Username.ToLower() && x.Password == model.Password);
        ...
        public IActionResult Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                if (_dbContext.Users.Any(x => x.Username.ToLower() == model.Username.ToLower()))
                {
                    ...
                    return View(model); //return for prevent duplicate username
                }
                string md5Salt = _configuration.GetValue<string>("AppSettings:MD5Salt");
                string saltedPassword = model.Password + md5Salt;
                string hashedPassword = saltedPassword.MD5();
